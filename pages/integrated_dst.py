...
if current_view == 'custom_analysis':
    ...
    if selected_step == 0:
        ...
        output = st_folium(map_object, height=600, width=1200,
        ...
        with button_col:
            if st.button("Extract Information", type="primary", key="extract_btn"):
                if st.session_state["last_polygon"] is None:
                    ...
                    st.session_state["extracted_data"] = None
        with reset_col:
            if st.button("Reset Polygon", key="reset_poly_btn"):
                reset_polygon()
        ...
        if st.session_state["extract_clicked"]:
                ...
                try:
                    ...
                    with st.status("Extracting and analyzing regional data...", expanded=True) as status:
                        ...
                        if response is None or response.status_code != 200:
                            st.session_state["extract_clicked"] = False; st.stop()
                        data = response.json()
                                ...
                                for filter_str in infra_options[infra]:
                                    ...
                                    if m:
                                        ...
                                        if k in tags and (v is None or tags[k] == v):
                                            filtered_elements.append(element); seen_ids.add(eid); break
                        ...
                        if st.session_state.get("gemini_client"):
                            ...
                            context_report = generate_context_report(center_lat, center_lon, area_sq_km, filtered_elements, selected_infras)
                        koppen_report = ""
                        if st.session_state.get("gemini_client"):
                    ...
                    st.session_state["extracted_data"] = {
                        ...
                        "context_report": context_report, "koppen_report": koppen_report,
                        "koppen_code": center_koppen_code, "center_lat": center_lat, "center_lon": center_lon,
                    }
                except Exception as e:
            ...
            if st.session_state["extracted_data"]:
                            ...
                            for element in elements:
                                ...
                                for filter_str in infra_options[infra]:
                                    ...
                                    if match:
                                        k, v = match.groups()
                                        if k in tags and (v is None or tags[k] == v):
                                            infra_elements.append(element); break
                            if infra_elements:
                                has_raw_data = True
                ...
                if center_lat is not None and center_lon is not None:
                    ...
                    if isinstance(plot_result, str): st.error(plot_result)
                    else: st.pyplot(plot_result)
                else:
                    st.warning("Cannot display Köppen map: Center coordinates could not be determined.")
                ...
                if koppen_report:
                    ...
                    with st.expander("📊 View Raw Data Fed to AI (Köppen Climate Code)", expanded=False):
                        ...
                        st.markdown("The code was derived from the Köppen-Geiger Climate Classification raster (1991–2020).")
    ...
    # ── STEP 1: Perceived Risks ────────────────────────────────
        ...
        use_previous_poly = st.checkbox("Use same polygon from 'Information Extraction and Mapping' section",
                                         value=has_lvl0_polygon, disabled=not has_lvl0_polygon)
        ...
        if lvl1_map_output and lvl1_map_output.get("last_active_drawing"):
            ...
            if drawing.get("geometry"):
                ...
                coords_d = drawing["geometry"]["coordinates"][0]
                lats_d = [c[1] for c in coords_d]; lons_d = [c[0] for c in coords_d]
                st.session_state["lvl1_bbox"] = [min(lats_d),min(lons_d),max(lats_d),max(lons_d)]

        col_scope1, col_scope2 = st.columns(2)
        with col_scope1:
            dynamic_infras = sorted(df_lvl2_base["Infrastructure"].unique().tolist()) if not df_lvl2_base.empty and "Infrastructure" in df_lvl2_base.columns else ["Road","Railway","Tunnels","Bridges","green spaces","Dams","River training infrastructure","Torrent control infrastructure"]
            selected_infra_type = st.selectbox("Select Infrastructure Type", options=dynamic_infras)
        with col_scope2:
        ...
        if not df_lvl2_base.empty and "Infrastructure" in df_lvl2_base.columns:
            ...
            available_impact_models = []; available_assets = []

        all_hazards_list_lvl1 = ["Extreme high temperatures (Heatwave)","Extreme cold temperatures (Coldwave, cold snap)","Drought","Wildfire (Forest fire or Bush fire)","Desertification","Storms & strong winds","Hail","Aeolian erosion","Pluvial flood, heavy rainfall and surface runoff","Fluvial flood","Coastal flood (e.g. storm surge)","Impact floods and Tsunami","Fluvial sediment transport","Stream bank & bed erosion","Sheet erosion & rill erosion","Gully erosion","Coastal and shoreline erosion (includes freshwater environments)","Debris flood (Volumetric Sediment Concentration 20-40%)","Debris flow (Volumetric Sediment Concentration >40%)","Small Rockfall (Diameter <25cm)","Large Rockfall (Diameter >25-100 cm)","Landslides < 2 m depth","Landslides 2-10 m depth","Landslides > 10 m depths","Mud or Earth flow","Soil slope deformation & Soil creep","Snow avalanches","Snow drift","Snow creep & slide"]
        ...
        dynamic_primaries = set(); dynamic_supportives = set()
        ...
        if st.button("➕ Add to Scope Table", type="primary", use_container_width=True):
            if selected_impact_model and selected_asset:
                ...
                base_row = {"Infrastructure":selected_infra_type,"Climate driver":selected_hazard_type,"Asset":selected_asset,"Impact model":selected_impact_model,"Possible Hazards":hazards_str}
                new_rows = []
                if not selected_primary_nbs and not selected_supportive_nbs:
                    row = base_row.copy(); row["NbS Type"]="None"; row["NbS Solution"]="None"; new_rows.append(row)
                else:
                    for nbs in selected_primary_nbs:
                        row = base_row.copy(); row["NbS Type"]="Primary"; row["NbS Solution"]=nbs; new_rows.append(row)
                    for nbs in selected_supportive_nbs:
                        row = base_row.copy(); row["NbS Type"]="Supportive"; row["NbS Solution"]=nbs; new_rows.append(row)
                st.session_state["lvl1_added_items"] = pd.concat([st.session_state["lvl1_added_items"], pd.DataFrame(new_rows)], ignore_index=True)
        ...
        if not st.session_state["lvl1_added_items"].empty:
            ...
            if "Risk Score" in display_df.columns: display_df = display_df.drop(columns=["Risk Score"])
            selection_event = st.dataframe(display_df, use_container_width=True, hide_index=True, on_select="rerun", selection_mode="multi-row", key="lvl1_scope_table")
            btn_col1, btn_col2, _ = st.columns([1.5,1.5,3])
            with btn_col1:
                if st.button("❌ Remove Selected", type="secondary", use_container_width=True):
                    selected_indices = selection_event.selection.rows
                    if selected_indices:
                        st.session_state["lvl1_added_items"] = st.session_state["lvl1_added_items"].drop(selected_indices).reset_index(drop=True); st.rerun()
                    else:
                        st.warning("Please select at least one row.")
            with btn_col2:
                if st.button("🗑️ Clear Scope Table", type="secondary", use_container_width=True):
                    st.session_state["lvl1_added_items"] = pd.DataFrame(columns=["Infrastructure","Climate driver","Asset","Impact model","Possible Hazards","NbS Type","NbS Solution"]); st.rerun()
        ...
        if "lvl1_added_items" not in st.session_state or st.session_state["lvl1_added_items"].empty:
            ...
            scope_options = st.session_state["lvl1_added_items"].apply(
                lambda r: f"{r['Infrastructure']} | {r['Asset']} | {r['Impact model']} | {r['Climate driver']} | {r.get('NbS Solution','None')}", axis=1).tolist()
            ...
            if selected_rating_item not in st.session_state["item_risk_matrices"]:
                st.session_state["item_risk_matrices"][selected_rating_item] = pd.DataFrame({sk:{k:3 for k in kpis} for sk in scenarios}, index=kpis).to_dict()
            if selected_rating_item not in st.session_state["item_loss_matrices"]:
                st.session_state["item_loss_matrices"][selected_rating_item] = pd.DataFrame({sk:{k:3 for k in kpis} for sk in scenarios}, index=kpis).to_dict()

            df_risk_data = st.session_state["item_risk_matrices"][selected_rating_item]
            df_risk = pd.DataFrame(df_risk_data, index=kpis); df_risk.index.name = "KPI / Indicator"
            st.info("Please select values between **1 (best/lowest)** and **5 (worst/highest)**.")

            matrix_tab, scenario_key_tab = st.tabs(["Input Matrices (1-5)","Scenario & KPI Definitions"])
            with scenario_key_tab:
                st.markdown("### Key Performance Indicators (KPIs)")
                st.markdown("* **SRS:** Safety, Reliability, and Security.\n* **AM:** Availability and Maintainability.\n* **EC:** Economy.\n* **EV:** Environment.\n* **HP:** Health and Politics.")
                st.markdown("### Scenario Definitions")
                for abbr, desc in scenarios.items(): st.markdown(f"**{abbr}**: {desc}")

            with matrix_tab:
                risk_column_config = {"KPI / Indicator": st.column_config.TextColumn("KPI / Indicator", disabled=True)}
                for key, desc in scenarios.items():
                    risk_column_config[key] = st.column_config.SelectboxColumn(label=key, help=desc, options=[1,2,3,4,5,"Not Available"], default=3, width="small", required=True)
                st.markdown("### 1. Risk Rating (Critical infrastructure Condition)")
                with st.form("risk_rating_form"):
                    risk_edited_df = st.data_editor(df_risk, column_config=risk_column_config, num_rows="fixed", key=f"risk_matrix_editor_{selected_rating_item}")
                ...
                if submit_risk:
                    ...
                    ci_hn_series = pd.to_numeric(risk_edited_df.get("CI_HN",pd.Series()), errors="coerce")
                    final_score = int(ci_hn_series.max()) if ci_hn_series.notna().any() else 0
                    temp_df = st.session_state["lvl1_added_items"].copy()
                    if "Risk Score" not in temp_df.columns: temp_df["Risk Score"] = None
                    mask = temp_df.apply(lambda r: f"{r['Infrastructure']} | {r['Asset']} | {r['Impact model']} | {r['Climate driver']} | {r.get('NbS Solution','None')}", axis=1) == selected_rating_item
                    if mask.any():
                        temp_df.loc[mask,"Risk Score"] = final_score
                        st.session_state["lvl1_added_items"] = temp_df
                        st.success(f"Saved! Risk Score (max CI_HN) = **{final_score}**")
                    st.rerun()

                st.markdown("### 2. Extent of Loss Rating (CI)")
                df_loss_data = st.session_state["item_loss_matrices"][selected_rating_item]
                df_loss = pd.DataFrame(df_loss_data, index=kpis); df_loss.index.name = "KPI / Indicator"
                loss_display_columns = [col for col in df_loss.columns if col != "CI"]
                df_loss_display = df_loss[loss_display_columns]
                column_config_loss = {"KPI / Indicator": st.column_config.TextColumn("KPI / Indicator", disabled=True)}
                for key, desc in scenarios.items():
                    if key != "CI":
                        column_config_loss[key] = st.column_config.SelectboxColumn(label=key, help=desc, options=[1,2,3,4,5], default=3, width="small")
                with st.form("loss_rating_form"):
                    loss_edited_df_display = st.data_editor(df_loss_display, column_config=column_config_loss, num_rows="fixed", key=f"loss_matrix_editor_{selected_rating_item}")
                    submit_loss = st.form_submit_button("Save Loss Ratings")
                if submit_loss:
                    if "CI" in df_loss.columns:
                        reconstructed_df_loss = loss_edited_df_display.assign(CI=df_loss["CI"].copy())
                    ...
                    st.success(f"Loss Ratings saved for: {selected_rating_item}"); st.rerun()
            ...
            if available_series:
                ...
                for i, s in enumerate(available_series):
                    with cols_radar[i]:
                        if st.checkbox(s, value=True, key=f"radar_checkbox_{s}"): selected_series.append(s)
                if selected_series:
                    radar_fig = create_radar_chart_plotly(kpis_for_plot, selected_series, "Risk Radar - Risk Ratings (X)")
            ...
            for i, scenario in enumerate(scenarios_to_plot):
                try:
                    fig = create_risk_heatmap_plotly(df_for_plot_risk, df_for_plot_loss, scenario, kpis)
                    with plot_cols[i % 2]: st.plotly_chart(fig, use_container_width=True)
                except Exception as e:
                    with plot_cols[i % 2]: st.error(f"Failed to generate plot for scenario {scenario}: {e}")

            st.subheader("Summary: Assessed Risks and Scores")
            if not st.session_state["lvl1_added_items"].empty:
                st.dataframe(st.session_state["lvl1_added_items"], use_container_width=True, hide_index=True)

            with st.expander("Interpretation"):
                if st.session_state.get("gemini_client"):
                    if st.button("Generate Interpretation Report", type="primary"):
                        try:
                            ...
                            with st.spinner("Generating Risk Matrix Interpretation (Gemini)..."):
                                interp = generate_risk_interpretation(current_df, kpis, scenarios)
                            st.session_state["interpretation_report"] = interp
                            render_ai_header(f"Risk Matrix Interpretation Report: {selected_rating_item.split(' | ')[0]}")
                            with st.expander("📊 View Raw Data Fed to AI", expanded=False): st.dataframe(current_df, use_container_width=True)
                            st.markdown(interp); render_ai_footer()
                        except Exception as e: st.error(f"Error: {e}")
                    elif st.session_state.get("interpretation_report"):
                        render_ai_header("Risk Matrix Interpretation Report")
                        st.markdown(st.session_state["interpretation_report"]); render_ai_footer()
                else:
                    st.warning("Gemini client not initialized. AI interpretation feature disabled.")
        ...
        primary_sols = {}; supp_sols = {}; has_valid_hazards = False
        if not st.session_state["lvl1_added_items"].empty:
            for _, row in st.session_state["lvl1_added_items"].iterrows():
                nbs_type = row.get("NbS Type","None"); sol = row.get("NbS Solution","None")
                hazards_str = str(row.get("Possible Hazards",""))
                if hazards_str != "None" and hazards_str.strip(): has_valid_hazards = True
                if sol == "None" or not str(sol).strip(): continue
                hazards = [h.strip() for h in hazards_str.split(",") if h.strip() and h.strip() != "None"]
                if nbs_type == "Primary":
                    if sol not in primary_sols: primary_sols[sol] = set()
                    primary_sols[sol].update(hazards)
                elif nbs_type == "Supportive":
                    if sol not in supp_sols: supp_sols[sol] = set()
                    supp_sols[sol].update(hazards)

        if not has_valid_hazards:
            st.warning("⚠️ No hazards have been identified in the Scope Table yet.")
        elif not primary_sols and not supp_sols:
            st.info("ℹ️ No specific NbS Solutions were added to the Scope Table.")

        primary_sols = {sol: sorted(list(hz)) for sol, hz in primary_sols.items()}
        supp_sols    = {sol: sorted(list(hz)) for sol, hz in supp_sols.items()}

        def build_lvl1_nbs_rows(sols_dict):
            return [{"Include":True,"NbS Solution":sol,"Addressed Hazards":", ".join(haz_list),"Technical Efficacy":3,"Implementation Costs":3,"Implementation Time":3,"Environmental Impact":3,"Social Acceptance":3} for sol, haz_list in sorted(sols_dict.items())]
        ...
        if primary_sols:
            ...
            with st.form("lvl1_primary_nbs_form"):
                edited_primary_lvl1 = st.data_editor(st.session_state["lvl1_primary_nbs_df"], column_config=nbs_lvl1_column_config, hide_index=True, use_container_width=True, key="lvl1_primary_nbs_editor")
                btn_col1, btn_col2 = st.columns(2)
                with btn_col1: save_prim = st.form_submit_button("Save Primary NbS Criteria", type="primary", use_container_width=True)
                with btn_col2: remove_prim = st.form_submit_button("❌ Remove Checked Rows", use_container_width=True)
            if save_prim:
                st.session_state["lvl1_primary_nbs_df"] = edited_primary_lvl1; st.success("Primary NbS criteria saved!"); st.rerun()
            if remove_prim:
                st.session_state["lvl1_primary_nbs_df"] = edited_primary_lvl1[~edited_primary_lvl1["Include"]].reset_index(drop=True); st.success("Checked rows removed!"); st.rerun()
        ...
        if supp_sols:
            ...
            with st.form("lvl1_supp_nbs_form"):
                edited_supp_lvl1 = st.data_editor(st.session_state["lvl1_supp_nbs_df"], column_config=nbs_lvl1_column_config, hide_index=True, use_container_width=True, key="lvl1_supp_nbs_editor")
                btn_col1, btn_col2 = st.columns(2)
                with btn_col1: save_supp = st.form_submit_button("Save Supportive NbS Criteria", type="primary", use_container_width=True)
                with btn_col2: remove_supp = st.form_submit_button("❌ Remove Checked Rows", use_container_width=True)
            if save_supp:
                st.session_state["lvl1_supp_nbs_df"] = edited_supp_lvl1; st.success("Supportive NbS criteria saved!"); st.rerun()
            if remove_supp:
                st.session_state["lvl1_supp_nbs_df"] = edited_supp_lvl1[~edited_supp_lvl1["Include"]].reset_index(drop=True); st.success("Checked rows removed!"); st.rerun()

        # ─ NbS Filtration (Level 1) ─
        st.markdown("---")
        ...
        if "lvl1_primary_nbs_df" in st.session_state and not st.session_state["lvl1_primary_nbs_df"].empty:
            approved_primary = set(st.session_state["lvl1_primary_nbs_df"][st.session_state["lvl1_primary_nbs_df"]["Include"]==True]["NbS Solution"])
        approved_supportive = set()
        if "lvl1_supp_nbs_df" in st.session_state and not st.session_state["lvl1_supp_nbs_df"].empty:
            approved_supportive = set(st.session_state["lvl1_supp_nbs_df"][st.session_state["lvl1_supp_nbs_df"]["Include"]==True]["NbS Solution"])
        ...
        if _lvl1_sei_dropdown_options:
            selected_nbs_for_sei_lvl1 = st.selectbox("Select an NbS Solution to configure SEI factors:", _lvl1_sei_dropdown_options, key="lvl1_sei_dropdown")
            if selected_nbs_for_sei_lvl1 not in st.session_state.lvl1_sei_lookup:
                st.session_state.lvl1_sei_lookup[selected_nbs_for_sei_lvl1] = {f:1 for f in ["Community Engagement","Cultural Preferences","Workforce Availability","Economic Viability","Long term O&M costs","Land Ownership","Regulatory Constraints"]}
        else:
            selected_nbs_for_sei_lvl1 = None

        ssf_labels = {"Slope instability":"Unstable/Steep Slopes","Limited vegetation and low quality of soil":"Poor Soil/Low Vegetation","Limited access for implementation":"Difficult Site Access","Cold temperatures":"Cold Temperatures / Frost Risk","Limited water availability":"Water Scarcity","Lack of connection to major services":"No Infrastructure/Services Access","Space Constraints":"Very Limited Space","Exposure to soil, water and/or air pollution":"Polluted Soil/Water/Air","Limitations due to high population density":"Urban/Densely Populated Area"}
            ...
            with col_input1:
                with st.container(border=True):
                    ...
                    for crit, label in ssf_labels.items():
                        val = st.toggle(label, value=st.session_state.global_site_conditions.get(crit,False), key=f"lvl1_ssf_global_cond_{crit}")
                        st.session_state.global_site_conditions[crit] = val
            with col_input2:
                with st.container(border=True):
                    st.markdown("##### 👥 Socio-Economic & Institutional (SEI)")
                    if selected_nbs_for_sei_lvl1:
                        st.caption(f"Configuring SEI factors for: **{selected_nbs_for_sei_lvl1}**")
                        for f in sei_factors_list:
                            current_val = st.session_state.lvl1_sei_lookup[selected_nbs_for_sei_lvl1].get(f,1)
                            val = st.select_slider(f, options=[1,2,3], value=current_val, key=f"lvl1_sei_{selected_nbs_for_sei_lvl1}_{f}", help="1: Favorable, 2: Neutral, 3: Unfavorable")
                            st.session_state.lvl1_sei_lookup[selected_nbs_for_sei_lvl1][f] = val
                    else:
                        st.info("Select an NbS solution from the dropdown above to configure SEI factors.")

        # Compute filtration
        st.session_state.lvl1_filtered_nbs_pool = []; lvl1_low_perf_pool = []
        source_df = st.session_state.get("lvl1_added_items", pd.DataFrame())
        lvl1_unique_pairs = {}; lvl1_method_to_hazards = {}

        for row_idx, row in source_df.iterrows():
            try: pri_score = float(row.get("Risk Score",0)) if row.get("Risk Score") is not None and str(row.get("Risk Score","")).strip() not in ["None",""] else 0.0
            except: pri_score = 0.0
            asset_val = str(row.get("Asset","")).strip() or "—"; impact_val = str(row.get("Impact model","")).strip() or "—"
            infra_val = str(row.get("Infrastructure","")).strip() or ""; row_label = f"{asset_val} — {impact_val}"
            raw_hazards = str(row.get("Possible Hazards",""))
            hazards = [h.strip() for h in raw_hazards.split(",") if h.strip() and h.strip() != "None"] if raw_hazards and raw_hazards != "None" else []
            sol = str(row.get("NbS Solution","")).strip(); nbs_type = str(row.get("NbS Type","")).strip()
            ...
            for h in hazards:
                for sol in all_sols:
                    pair_name = f"{sol} method for {h}"; up_key = (row_idx, pair_name); is_prim = sol in row_p_sols
                    if up_key not in lvl1_unique_pairs:
                        lvl1_unique_pairs[up_key] = {"original_pri":pri_score,"status":"scored","pair_name":pair_name,"method_only":sol,"row_label":row_label,"asset":asset_val,"impact_model":impact_val,"infrastructure":infra_val,"is_primary":is_prim}
                    elif lvl1_unique_pairs[up_key]["original_pri"] < pri_score:
                        lvl1_unique_pairs[up_key]["original_pri"] = pri_score
                        if is_prim: lvl1_unique_pairs[up_key]["is_primary"] = True
                    if sol not in lvl1_method_to_hazards: lvl1_method_to_hazards[sol] = set()
                    lvl1_method_to_hazards[sol].add(h)
        ...
        for _, data in lvl1_unique_pairs.items():
            ...
            hia_scores = []
            for haz_key in all_lvl1_hazards:
                haz_data = nbs_db.get(haz_key, {})
                if method_base in haz_data.get("Yes",[]): hia_scores.append(100)
                elif method_base in haz_data.get("Supportive",[]): hia_scores.append(50)
                else: hia_scores.append(0)
            ...
            af = 1.0 - tech_eff / 5.0; rpri = data.get("original_pri",0) * af
            data.update({"name":data["pair_name"],"ssf":avg_ssf,"sei":avg_sei,"hia":avg_hia,"total":total_score,"tech_eff":tech_eff,"eff_percent":total_score,"rpri":rpri})
            if total_score > 33.34: st.session_state.lvl1_filtered_nbs_pool.append(data)
            else: lvl1_low_perf_pool.append(data)

        if not st.session_state.lvl1_filtered_nbs_pool and lvl1_unique_pairs:
            st.warning("⚠️ No solutions strictly passed the 33.34% feasibility threshold. Showing all potential methods.")
            st.session_state.lvl1_filtered_nbs_pool = list(lvl1_unique_pairs.values())

        # Compute suitability scores for Level 1
        lvl1_method_max_total = {}
        for data_dict in lvl1_unique_pairs.values():
            m_name = str(data_dict.get("method_only","")).strip()
            t_score = data_dict.get("total",0.0)
            if m_name and (m_name not in lvl1_method_max_total or t_score > lvl1_method_max_total[m_name]):
                lvl1_method_max_total[m_name] = t_score

        def get_lvl1_si(nbs_name, row_data):
            final_index = lvl1_method_max_total.get(nbs_name, 100.0)
            if final_index <= 33.34: return 0.0
            factors = [row_data.get("Technical Efficacy",3),row_data.get("Implementation Costs",3),row_data.get("Implementation Time",3),row_data.get("Environmental Impact",3),row_data.get("Social Acceptance",3)]
            for f in factors:
                if f == "Not Acceptable" or pd.isna(f): return 0.0
            try:
                numeric_factors = [float(f) for f in factors]
                if any(pd.isna(x) for x in numeric_factors): return 0.0
        ...
        lvl1_si_dict = {}
        for df_key, df_attr in [("lvl1_primary_nbs_df",True),("lvl1_supp_nbs_df",False)]:
            if df_key in st.session_state and not st.session_state[df_key].empty:
                for _, row in st.session_state[df_key].iterrows():
                    if row.get("Include"):
                        m_name = str(row.get("NbS Solution","")).strip()
                        lvl1_si_dict[m_name] = get_lvl1_si(m_name, row)

        for item in st.session_state.lvl1_filtered_nbs_pool:
            m_name = item.get("method_only","")
            item["si"] = lvl1_si_dict.get(m_name,0.0)
        ...
        if st.session_state.lvl1_filtered_nbs_pool:
            ...
            df_table = df_strat[["infrastructure","asset","impact_model","method_only","original_pri","si","suitability_score"]].copy()
            df_table.rename(columns={"infrastructure":"Infrastructure","asset":"Asset","impact_model":"Impact model","method_only":"NbS Solution","original_pri":"Risk Score","si":"Suitability Index (SI)","suitability_score":"Suitability Score (NbS)"}, inplace=True)
            df_table = df_table.drop_duplicates().reset_index(drop=True)
            st.dataframe(df_table, column_config={"Suitability Index (SI)":st.column_config.NumberColumn(format="%.3f"),"Suitability Score (NbS)":st.column_config.NumberColumn(format="%.2f"),"Risk Score":st.column_config.NumberColumn(format="%.1f")}, use_container_width=True, hide_index=True)

            st.divider()
            df_final_lvl1 = pd.DataFrame(st.session_state.lvl1_filtered_nbs_pool).sort_values(by="total",ascending=False).drop_duplicates(subset=["name"])
            fig_spider_lvl1 = go.Figure()
            for _, r in df_final_lvl1.head(5).iterrows():
                fig_spider_lvl1.add_trace(go.Scatterpolar(r=[r["ssf"],r["sei"],r["hia"],r["ssf"]],theta=["Site Feasibility (SSF)","Socio-Economic (SEI)","Hazard Attenuation (HIA)","Site Feasibility (SSF)"],fill="toself",name=r["name"]))
            fig_spider_lvl1.update_layout(polar=dict(radialaxis=dict(visible=True,range=[0,100])),showlegend=True)
            st.plotly_chart(fig_spider_lvl1, use_container_width=True)

            st.markdown("##### 🏆 Ranked NbS Solutions by Suitability Score")
            seen_labels_lvl1 = []
            for item in st.session_state.lvl1_filtered_nbs_pool:
                lbl = item.get("row_label","")
                if lbl and lbl not in seen_labels_lvl1: seen_labels_lvl1.append(lbl)

            OPT_PRIMARY = "✅ All Primary Solutions"; OPT_SUPPORTIVE = "🔄 All Supportive Solutions"; OPT_ALL = "✨ All Solutions"
            view_options = [OPT_PRIMARY, OPT_SUPPORTIVE, OPT_ALL] + seen_labels_lvl1
            st.info("Use the selector below to view rankings.", icon="ℹ️")
            selected_view_lvl1 = st.selectbox("📋 View ranking for:", view_options, key="si_view_selector")

            def _color_si(score_val, min_val, max_val):
            ...
            def render_suitability_ranking(pool, solution_filter="primary", show_row_badge=False):
                ...
                for item in pool:
                    ...
                    if solution_filter=="primary" and not is_p: continue
                    if solution_filter=="supportive" and is_p: continue
                    m = item["method_only"]
                    if m not in cons or item.get("suitability_score",0) > cons[m].get("suitability_score",0): cons[m] = item
                scored = sorted([i for i in cons.values() if i["status"]=="scored"], key=lambda x:x["suitability_score"], reverse=True)
                if not scored: st.info(f"No {solution_filter} solutions found."); return
                valid_vals = [i["suitability_score"] for i in scored]
                mn, mx = (min(valid_vals),max(valid_vals)) if valid_vals else (0,0)
                for rank, item in enumerate(scored, 1):
                    bg = _color_si(item["suitability_score"],mn,mx); tc = "black" if bg in ["#fff176","#ffb74d","#aed581"] else "white"
                    score_val = item.get("suitability_score",0.0)
                    type_tag = ""; row_badge_html = ""
                    if solution_filter=="all":
                        is_p = item.get("is_primary",False); t_color = "#2d6a4f" if is_p else "#1565c0"; t_label = "Primary" if is_p else "Supportive"
                        type_tag = f'<span style="background:{t_color};color:white;border-radius:4px;padding:1px 6px;font-size:0.7em;margin-right:8px;">{t_label}</span>'
                    if show_row_badge:
                        lbl = item.get("row_label","")
                    ...
                    st.markdown(f'<div style="background:#ffffff;border:1.5px solid #dee2e6;border-left:5px solid {bg};border-radius:8px;padding:12px 16px;margin-bottom:10px;display:flex;align-items:center;gap:16px;box-shadow:0 1px 4px rgba(0,0,0,0.07);"><div style="background:{bg};color:{tc};border-radius:50%;min-width:40px;height:40px;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:18px;flex-shrink:0;">{rank}</div><div style="flex:1;color:#212529;"><div style="font-weight:600;font-size:1em;margin-bottom:3px;">{type_tag}{item["method_only"]}{row_badge_html}</div><div style="font-size:0.9em;">🔹 <b>Suitability Score:</b> {score_val:.2f} &nbsp;|&nbsp; <b>Total Feasibility: {item["eff_percent"]:.1f}%</b></div></div></div>', unsafe_allow_html=True)

            if selected_view_lvl1 == OPT_PRIMARY: render_suitability_ranking(st.session_state.lvl1_filtered_nbs_pool,"primary",True)
            elif selected_view_lvl1 == OPT_SUPPORTIVE: render_suitability_ranking(st.session_state.lvl1_filtered_nbs_pool,"supportive",True)
            elif selected_view_lvl1 == OPT_ALL: render_suitability_ranking(st.session_state.lvl1_filtered_nbs_pool,"all",True)
            else:
                row_pool_lvl1 = [item for item in st.session_state.lvl1_filtered_nbs_pool if item.get("row_label")==selected_view_lvl1]
                render_suitability_ranking(row_pool_lvl1,"all",False)
        else:
    ...
    # ── STEP 2: Technical Analysis ─────────────────────────────
        ...
        infrastructure_col = "Infrastructure"; climate_driver_col = "Climate driver"
        type_impact_col = "Type of impact"; impact_model_col = "Impact model"
        ...
        if df_lvl2_base.empty:
            ...
            if st.button("Add filtered items to Table", type="primary", use_container_width=True):
                if not final_filtered_df.empty:
                    st.session_state.saved_data = pd.concat([st.session_state.saved_data, final_filtered_df], ignore_index=True).drop_duplicates()
        ...
        with btn_col1:
            if st.button("🗑️ Reset Table", type="secondary", use_container_width=True):
                ...
                st.session_state.capex_df = pd.DataFrame(); st.rerun()
        with btn_col3:
            with st.popover("➕ Add Custom Impact Model", use_container_width=True):
                st.markdown("##### Define Custom Impact Model")
                c1, c2 = st.columns(2)
                with c1:
                    new_infra = st.selectbox("Infrastructure Category",["Road","Railway","Tunnels","Bridges","green spaces","Dams","River training infrastructure","Torrent control infrastructure"],key="manual_infra_sel")
                ...
                with c2:
                    ...
                    all_hazards_list_manual = ["Extreme high temperatures (Heatwave)","Extreme cold temperatures (Coldwave, cold snap)","Drought","Wildfire (Forest fire or Bush fire)","Desertification","Storms & strong winds","Hail","Aeolian erosion","Pluvial flood, heavy rainfall and surface runoff","Fluvial flood","Coastal flood (e.g. storm surge)","Impact floods and Tsunami","Fluvial sediment transport","Stream bank & bed erosion","Sheet erosion & rill erosion","Gully erosion","Coastal and shoreline erosion (includes freshwater environments)","Debris flood (Volumetric Sediment Concentration 20-40%)","Debris flow (Volumetric Sediment Concentration >40%)","Small Rockfall (Diameter <25cm)","Large Rockfall (Diameter >25-100 cm)","Landslides < 2 m depth","Landslides 2-10 m depth","Landslides > 10 m depths","Mud or Earth flow","Soil slope deformation & Soil creep","Snow avalanches","Snow drift","Snow creep & slide"]
                    new_hazards = st.multiselect("Select Possible Hazards",options=all_hazards_list_manual,key="manual_haz_sel")
                if st.button("Confirm and Add to Table",type="primary",use_container_width=True):
                    if new_impact.strip() and new_asset.strip():
                        ...
                        new_row = {infrastructure_col:new_infra,climate_driver_col:new_driver,impact_model_col:new_impact,"Asset":new_asset,dictionary_key_col:new_dict_key,"Type of impact":"Custom","Consequences":"Custom","Proposed climate Indicator":new_indicator_name,"Possible Hazards":new_hazards}
                        st.session_state.saved_data = pd.concat([st.session_state.saved_data,pd.DataFrame([new_row])],ignore_index=True)
                        st.toast(f"Added {new_asset}!",icon="✅"); st.rerun()
                    else:
        ...
        if not st.session_state.saved_data.empty:
            ...
            selection_event = st.dataframe(st.session_state.saved_data[cols_to_display], on_select="rerun", selection_mode="multi-row", use_container_width=True, key="impact_table_selection")
            with btn_col2:
                if st.button("❌ Remove Selected", type="secondary", use_container_width=True):
                    selected_indices = selection_event.selection.rows
                    if selected_indices:
                        st.session_state.saved_data = st.session_state.saved_data.drop(selected_indices).reset_index(drop=True); st.rerun()
                    else:
                        st.warning("Select rows to remove.")
        else:
            st.info("Table is empty. Use filters above or add a custom impact model.")

        st.divider()
        st.subheader("3. Hazard Variation Analysis")
        URL = "https://naturedemo-clima-ind.dic-cloudmate.eu/api/calculate"
        LEVEL_TO_INDEX = {"No variation":0,"Low":1,"Medium":2,"High":3,"Very High":4,"Extreme":5}
        INDEX_TO_LEVEL = {v:k for k,v in LEVEL_TO_INDEX.items()}
        has_extracted_coords = st.session_state.get("extracted_data") is not None and st.session_state["extracted_data"].get("center_lat") is not None
        use_poly_center = st.checkbox("Use Polygon Center Coordinates", value=has_extracted_coords, disabled=not has_extracted_coords)
        if use_poly_center and has_extracted_coords:
            default_lat = st.session_state["extracted_data"]["center_lat"]; default_lon = st.session_state["extracted_data"]["center_lon"]; input_disabled = True
        else:
            default_lat = st.session_state["map_center"][0] if st.session_state["map_center"] else 50.7764
            default_lon = st.session_state["map_center"][1] if st.session_state["map_center"] else 6.0839; input_disabled = False
        row1_c1, row1_c2 = st.columns(2)
        with row1_c1: lat = st.number_input("Latitude:",value=float(default_lat),format="%.4f",disabled=input_disabled)
        with row1_c2: selected_scenario_label = st.radio("Select Scenario:",("RCP4.5","RCP8.5"),horizontal=True)
        row2_c1, row2_c2 = st.columns(2)
        with row2_c1: lon = st.number_input("Longitude:",value=float(default_lon),format="%.4f",disabled=input_disabled)
        with row2_c2: selected_term_label = st.radio("Select Time Horizon:",("Short Term","Medium Term","Long Term"),horizontal=True)
        st.markdown(" ")
        if st.button("Calculate Hazard Variation & Level",type="primary",use_container_width=True):
            if st.session_state.saved_data.empty:
                ...
                scenario_map = {"RCP4.5":"rcp45","RCP8.5":"rcp85"}; term_map = {"Short Term":"short","Medium Term":"medium","Long Term":"long"}
                api_scenario = scenario_map[selected_scenario_label]; api_term = term_map[selected_term_label]
                index_list = []; level_list = []
                unique_keys = [k for k in st.session_state.saved_data[dictionary_key_col].unique() if k and str(k).strip() not in ("","Not found")]
                progress_bar = st.progress(0); status_text = st.empty(); key_cache = {}; fetch_count = 0
                for dict_key in unique_keys:
                    fetch_count += 1; val_selected = None; val_historical = None
                    try:
                        payload_sel = {
                            "index_type": dict_key,
                    ...
                    if val_selected is not None and val_historical is not None:
                        try:
                            variation_val = 0.0 if val_historical==0 and val_selected==0 else (float("inf") if val_historical==0 else abs((val_selected-val_historical)/val_historical)*100)
                    ...
                    key_cache[dict_key] = (cached_idx,cached_lvl); progress_bar.progress(fetch_count/len(unique_keys))
                status_text.empty()
                for _, row in st.session_state.saved_data.iterrows():
                    ...
                    index_list.append(final_idx); level_list.append(final_lvl)
                result_df = st.session_state.saved_data[[infrastructure_col,climate_driver_col,impact_model_col]].copy()
                result_df["Hazard Index"] = index_list; result_df["Hazard Level"] = level_list
                st.session_state.calculated_results = result_df; st.success("Calculation complete!"); st.rerun()

        if not st.session_state.calculated_results.empty:
            st.subheader("Natural Hazards Table")
            if st.button("Delete rows without climate information",type="primary"):
                before = len(st.session_state.calculated_results)
                st.session_state.calculated_results = st.session_state.calculated_results[(st.session_state.calculated_results["Hazard Index"]!=0)&(st.session_state.calculated_results["Hazard Index"].notna())].reset_index(drop=True)
                st.toast(f"Deleted {before - len(st.session_state.calculated_results)} rows.",icon="🗑️"); st.rerun()
            ...
            column_config = {"Hazard Index":st.column_config.ProgressColumn("Hazard Index",min_value=0,max_value=5,format="%d"),"Hazard Level":st.column_config.SelectboxColumn("Hazard Level",options=["No variation","Low","Medium","High","Very High","Extreme"],required=True),infrastructure_col:st.column_config.TextColumn(disabled=True),climate_driver_col:st.column_config.TextColumn(disabled=True),impact_model_col:st.column_config.TextColumn(disabled=True)}
            temp_hazard_df = st.data_editor(display_df,column_config=column_config,use_container_width=True,hide_index=True,num_rows="fixed",key="hazard_editor_v3")
            if st.button("Save Hazard Changes",type="primary"):
                for i in temp_hazard_df.index:
                    old_level = display_df.loc[i,"Hazard Level"]; new_level = temp_hazard_df.loc[i,"Hazard Level"]
                    if new_level != old_level: temp_hazard_df.at[i,"Hazard Index"] = LEVEL_TO_INDEX.get(new_level,0)
                if temp_hazard_df["Hazard Index"].max()>5 or temp_hazard_df["Hazard Index"].min()<0:
                    st.error("Hazard Index values must be between 0 and 5.")
                else:
                    st.session_state.calculated_results.update(temp_hazard_df); st.success("Saved!"); st.rerun()
            st.markdown(
                """
            ...
            if st.button("Generate Climate Hazard Report",type="secondary",use_container_width=True):
                with st.spinner("Generating Report (Gemini)..."):
            ...
            if st.session_state.get("hazard_report"):
                with st.expander("View Climate Hazard Report",expanded=True):
                    ...
                    st.markdown(st.session_state["hazard_report"]); render_ai_footer()
        ...
        with col_exp1: economic_available = st.toggle("💰 Economic data for infrastructure assets are available",value=False)
        with col_exp2: use_default_thresholds = st.toggle("⚙️ Use default threshold values",value=True,disabled=not economic_available)

        def refresh_asset_list():
            if not st.session_state.calculated_results.empty:
                if "Asset" not in st.session_state.calculated_results.columns:
                    try: st.session_state.calculated_results["Asset"] = st.session_state.saved_data.loc[st.session_state.calculated_results.index,"Asset"]
                    except: pass
                unique_assets = st.session_state.calculated_results["Asset"].unique() if "Asset" in st.session_state.calculated_results.columns else st.session_state.saved_data["Asset"].unique()
                st.session_state.capex_df = pd.DataFrame({"Exposed assets":unique_assets,"CAPEX (M€/year)":0.0})

        if st.session_state.capex_df.empty: refresh_asset_list()
        exposure_val = 3
        if economic_available:
            with st.container(border=True):
                col_rev,col_btn = st.columns([2,1])
                with col_rev: revenue_input = st.number_input("Annual REVENUES (M€/year)",min_value=0.0,value=0.0,step=0.1)
                with col_btn:
                    st.markdown("<br>",unsafe_allow_html=True)
                    if st.button("🔄 Refresh Asset List",use_container_width=True): refresh_asset_list()
                st.markdown("##### Capital Expenditure (CAPEX) per Asset")
                if not st.session_state.capex_df.empty:
                    edited_capex = st.data_editor(st.session_state.capex_df,column_config={"Exposed assets":st.column_config.TextColumn("Exposed Asset",disabled=True),"CAPEX (M€/year)":st.column_config.NumberColumn("CAPEX (M€/year)",min_value=0.0,format="%.2f M€")},use_container_width=True,hide_index=True,key="capex_editor")
                else:
                    st.warning("No assets found. Run hazards calculation first."); edited_capex = pd.DataFrame()
            ...
            revenue_input = 0.0; rev_low,rev_high,cap_low,cap_high = 1.0,2.5,5.0,10.0; edited_capex = pd.DataFrame()
        ...
        if not st.session_state.calculated_results.empty and "Exposure Index" in st.session_state.calculated_results.columns:
            ...
            st.dataframe(st.session_state.calculated_results, column_config={"Exposure Index":st.column_config.ProgressColumn("Exposure Index",format="%d",min_value=0,max_value=5),"Sensitivity Index":None,"Vulnerability Index":None,"Asset":None,"PRI scores":None,"PRI values":None,"Hazard Index":None,"Hazard Level":None,"Possible Hazards":None}, use_container_width=True)
        ...
        if not st.session_state.calculated_results.empty:
            ...
            with st.container(border=True):
                ...
                vuln_col_config = {"Sensitivity Index":st.column_config.SelectboxColumn("Sensitivity Index (1-5)",options=[1,2,3,4,5],required=True),"Exposure Index":None,"Vulnerability Index":None,"PRI scores":None,"PRI values":None,"Hazard Index":None,"Hazard Level":None,"Infrastructure":st.column_config.TextColumn(disabled=True),"Climate driver":st.column_config.TextColumn(disabled=True),"Impact model":st.column_config.TextColumn(disabled=True),"Asset":st.column_config.TextColumn(disabled=True)}
                temp_edited_df = st.data_editor(vuln_input_df,column_config=vuln_col_config,use_container_width=True,hide_index=True,key="sensitivity_editor_v2")
            with st.container(border=True):
                ...
                asset_ac_params = {}; unique_assets_list = st.session_state.calculated_results.get("Asset",pd.Series()).unique() if "Asset" in st.session_state.calculated_results.columns else []
                if ac_available:
                    if st.session_state.get("ac_editor_df") is None or (hasattr(st.session_state.ac_editor_df,"__len__") and set(st.session_state.ac_editor_df.get("Asset",[]).tolist()) != set(unique_assets_list.tolist())):
                        st.session_state.ac_editor_df = pd.DataFrame({"Asset":list(unique_assets_list),"Initial AC (0-0.4)":[0.0]*len(unique_assets_list),"Lifetime":["Intermediate"]*len(unique_assets_list),"Maintenance":["Medium"]*len(unique_assets_list),"Topology":["Acceptable"]*len(unique_assets_list)})
                    ac_col_config = {"Asset":st.column_config.TextColumn("Asset",disabled=True),"Initial AC (0-0.4)":st.column_config.NumberColumn("Initial AC (0-0.4)",min_value=0.0,max_value=0.4,step=0.01,format="%.2f"),"Lifetime":st.column_config.SelectboxColumn("Lifetime",options=["Greenfield","Intermediate","High (> 25 years)"],required=True),"Maintenance":st.column_config.SelectboxColumn("Maintenance Level",options=["High","Medium","Low"],required=True),"Topology":st.column_config.SelectboxColumn("Design Topology",options=["Resilient","Acceptable","Not acceptable"],required=True)}
                    edited_ac_df = st.data_editor(st.session_state.ac_editor_df,column_config=ac_col_config,hide_index=True,use_container_width=True,key="ac_data_editor")
                    for _, row in edited_ac_df.iterrows():
                        a = row["Asset"]; s_ac0 = row["Initial AC (0-0.4)"]
                        v_lf = 10 if row["Lifetime"]=="Greenfield" else (-10 if row["Lifetime"]=="High (> 25 years)" else 0)
                        v_lm = 10 if row["Maintenance"]=="High" else (-10 if row["Maintenance"]=="Low" else 0)
                        v_dt = 10 if row["Topology"]=="Resilient" else (-10 if row["Topology"]=="Not acceptable" else 0)
            ...
            if st.button("Calculate Vulnerability Index",type="primary",use_container_width=True):
                ...
                for index, row in st.session_state.calculated_results.iterrows():
                    asset_name = row["Asset"]; sensitivity_val = row["Sensitivity Index"]; final_ac = 0.0
                    if ac_available and asset_name in asset_ac_params:
                        ...
                        final_ac = min(max(calculated_ac,0.0),0.4)
                    vulnerability_results.append(sensitivity_val * (1 - final_ac))
            ...
            if "Vulnerability Index" in st.session_state.calculated_results.columns:
                ...
                st.dataframe(st.session_state.calculated_results, column_config={"Sensitivity Index":st.column_config.NumberColumn("Sensitivity",format="%d"),"Vulnerability Index":st.column_config.ProgressColumn("Vulnerability Index",format="%.2f",min_value=0,max_value=5),"Exposure Index":None,"PRI scores":None,"PRI values":None,"Asset":None,"Hazard Index":None,"Possible Hazards":None,"Hazard Level":None}, use_container_width=True)
        else:
            st.warning("Please complete the previous sections (Filters → Hazards → Exposure) to generate data for analysis.")
        ...
        if st.button("Calculate the Potential Risk Index (PRI)",type="primary",use_container_width=True):
            if not st.session_state.calculated_results.empty:
                ...
                required_cols = ["Hazard Index","Exposure Index","Vulnerability Index"]
                if all(col in df_base.columns for col in required_cols):
                    pri_table = {0:{"score":0,"value":"NO RISK"},25:{"score":1,"value":"VERY LOW"},50:{"score":2,"value":"LOW"},75:{"score":3,"value":"MEDIUM"},100:{"score":4,"value":"HIGH"},125:{"score":5,"value":"EXTREME"}}
                    allowed_levels = list(pri_table.keys()); pri_scores = []; pri_values = []
                    for _, row in df_base.iterrows():
                        try:
                            h = float(row["Hazard Index"]) if pd.notnull(row["Hazard Index"]) else 0.0
                            v = float(row["Vulnerability Index"]) if pd.notnull(row["Vulnerability Index"]) else 0.0
                            e = float(row["Exposure Index"]) if pd.notnull(row["Exposure Index"]) else 0.0
                        except: h,v,e = 0.0,0.0,0.0
                        raw_product = h*v*e; lookup_key = min(allowed_levels,key=lambda x:abs(x-raw_product))
                        pri_scores.append(pri_table[lookup_key]["score"]); pri_values.append(pri_table[lookup_key]["value"])
                    df_base["PRI scores"] = pri_scores; df_base["PRI values"] = pri_values
                    st.session_state.calculated_results = df_base; st.session_state.pri_display_df = df_base.copy()
                    st.success("Potential Risk Index (PRI) calculated successfully!"); st.rerun()
                else:
        ...
        st.markdown("#### Step 7.1: NbS Selection")
        all_hazards_for_selector = ["Extreme high temperatures (Heatwave)","Extreme cold temperatures (Coldwave, cold snap)","Drought","Wildfire (Forest fire or Bush fire)","Desertification","Storms & strong winds","Hail","Aeolian erosion","Pluvial flood, heavy rainfall and surface runoff","Fluvial flood","Coastal flood (e.g. storm surge)","Impact floods and Tsunami","Fluvial sediment transport","Stream bank & bed erosion","Sheet erosion & rill erosion","Gully erosion","Coastal and shoreline erosion (includes freshwater environments)","Debris flood (Volumetric Sediment Concentration 20-40%)","Debris flow (Volumetric Sediment Concentration >40%)","Small Rockfall (Diameter <25cm)","Large Rockfall (Diameter >25-100 cm)","Landslides < 2 m depth","Landslides 2-10 m depth","Landslides > 10 m depths","Mud or Earth flow","Soil slope deformation & Soil creep","Snow avalanches","Snow drift","Snow creep & slide"]

        if st.button("🔄 Auto-extract Hazards from PRI Table",use_container_width=True):
            if "pri_display_df" in st.session_state and not st.session_state.pri_display_df.empty:
                if "Possible Hazards" in st.session_state.pri_display_df.columns:
                    extracted_hazards = set()
                    for _, row in st.session_state.pri_display_df.iterrows():
                    ...
                    st.success(f"Extracted {len(st.session_state.selected_nbs_hazards)} hazards."); st.rerun()
                else: st.error("Could not find hazard data in the PRI table.")
            else: st.error("No PRI calculation results available.")

        if not isinstance(st.session_state.selected_nbs_hazards,list): st.session_state.selected_nbs_hazards = []
        ...
        safe_indices = current_indices if current_indices else None
        if "hazard_transfer_key" not in st.session_state: st.session_state.hazard_transfer_key = 0
        ...
        if st.session_state.selected_nbs_hazards:
            for h in st.session_state.selected_nbs_hazards:
                if h in nbs_db: dynamic_nbs_list.update(nbs_db[h].get("Yes",[]))
        ...
        if "prev_dynamic_nbs" not in st.session_state or st.session_state.prev_dynamic_nbs != dynamic_nbs_list:
            st.session_state.prev_dynamic_nbs = dynamic_nbs_list; st.session_state.approved_nbs_methods = dynamic_nbs_list

        _HAZARD_ICONS = {"Heatwave":"🌡️","Coldwave":"🧊","Drought":"🏜️","Wildfire":"🔥","Desertification":"🌵","Storms":"🌪️","Hail":"🌨️","erosion":"⛰️","flood":"🌊","Flood":"🌊","sediment":"🪨","Rockfall":"🪨","Landslide":"⛰️","avalanche":"❄️","Snow":"❄️","Mud":"🌧️"}
        def _hazard_icon(name):
            for kw,icon in _HAZARD_ICONS.items():
                if kw.lower() in name.lower(): return icon
            return "⚠️"
            ...
            for h in st.session_state.selected_nbs_hazards:
                for s in nbs_db.get(h,{}).get(sol_key,[]):
                    if s in all_sols_list:
                        ...
                        if h not in sol_to_hazards[s]: sol_to_hazards[s].append(h)
            return [{"Include":True,"NbS Solution":sol,"Addressed Hazards":", ".join(_hazard_icon(h)+" "+h for h in hazards)} for sol,hazards in sorted(sol_to_hazards.items())]
        ...
        if st.session_state.selected_nbs_hazards and dynamic_nbs_list:
            ...
            if "nbs_table_excluded" not in st.session_state: st.session_state.nbs_table_excluded = set()
            current_primary_sols = {r["NbS Solution"] for r in primary_rows}
            st.session_state.nbs_table_excluded &= current_primary_sols
            for row in primary_rows: row["Include"] = row["NbS Solution"] not in st.session_state.nbs_table_excluded
            edited_primary = st.data_editor(pd.DataFrame(primary_rows),column_config={"Include":st.column_config.CheckboxColumn("Include",width="small"),"NbS Solution":st.column_config.TextColumn("NbS Solution",width="medium",disabled=True),"Addressed Hazards":st.column_config.TextColumn("Addressed Hazards",width="large",disabled=True)},hide_index=True,use_container_width=True,key="primary_nbs_table")
            if st.button("Save Primary NbS Selections",type="primary"):
                st.session_state.nbs_table_excluded = {row["NbS Solution"] for _,row in edited_primary.iterrows() if not row["Include"]}
                approved_nbs = sorted(edited_primary.loc[edited_primary["Include"],"NbS Solution"].tolist())
                st.session_state.approved_nbs_methods = approved_nbs; st.toast("Primary selections saved!",icon="✅"); st.rerun()
            st.markdown(f"<div style='background:#f1f8e9;border-left:4px solid #43a047;padding:8px 14px;border-radius:6px;margin-top:4px;color:#1b5e20;'><strong>Selection: {len(edited_primary[edited_primary['Include']])} / {len(primary_rows)}</strong></div>",unsafe_allow_html=True)

        dynamic_supportive_list = set()
        if st.session_state.selected_nbs_hazards:
            for h in st.session_state.selected_nbs_hazards:
                if h in nbs_db: dynamic_supportive_list.update(nbs_db[h].get("Supportive",[]))
        dynamic_supportive_list = sorted(list(dynamic_supportive_list))
        if "approved_supportive_methods" not in st.session_state: st.session_state.approved_supportive_methods = dynamic_supportive_list
        if "prev_dynamic_supportive" not in st.session_state or st.session_state.prev_dynamic_supportive != dynamic_supportive_list:
            st.session_state.prev_dynamic_supportive = dynamic_supportive_list; st.session_state.approved_supportive_methods = dynamic_supportive_list

        if st.session_state.selected_nbs_hazards and dynamic_supportive_list:
            show_supportive = st.toggle("🔄 Configure Supportive NbS Solutions",value=False)
            supportive_rows = _build_solution_table("Supportive",dynamic_supportive_list)
            if "nbs_supp_table_excluded" not in st.session_state: st.session_state.nbs_supp_table_excluded = set()
            current_supp_sols = {r["NbS Solution"] for r in supportive_rows}
            st.session_state.nbs_supp_table_excluded &= current_supp_sols
            for row in supportive_rows: row["Include"] = row["NbS Solution"] not in st.session_state.nbs_supp_table_excluded
            if show_supportive:
                edited_supportive = st.data_editor(pd.DataFrame(supportive_rows),column_config={"Include":st.column_config.CheckboxColumn("Include",width="small"),"NbS Solution":st.column_config.TextColumn("NbS Solution",width="medium",disabled=True),"Addressed Hazards":st.column_config.TextColumn("Addressed Hazards",width="large",disabled=True)},hide_index=True,use_container_width=True,key="supportive_nbs_table")
                if st.button("Save Supportive NbS Selections",type="primary"):
                    st.session_state.nbs_supp_table_excluded = {row["NbS Solution"] for _,row in edited_supportive.iterrows() if not row["Include"]}
                    st.session_state.approved_supportive_methods = sorted(edited_supportive.loc[edited_supportive["Include"],"NbS Solution"].tolist())
                    st.toast("Supportive selections saved!",icon="✅"); st.rerun()
        ...
        if not st.session_state.calculated_results.empty:
            ...
            primary_sol_col = []; supportive_sol_col = []
            for _, row in summary_display_df.iterrows():
                hazards_item = row.get("Possible Hazards",[])
                if isinstance(hazards_item,str): current_hazards = [h for h in all_hazards_for_selector if h in hazards_item]
                else: current_hazards = [h for h in (hazards_item if isinstance(hazards_item,list) else []) if h in all_hazards_for_selector]
                p_sols,s_sols = set(),set()
                for h in current_hazards:
                    h_strip = h.strip()
                    if h_strip in nbs_db:
                        if h_strip in st.session_state.selected_nbs_hazards:
                            p_sols.update([s for s in nbs_db[h_strip].get("Yes",[]) if s in approved_nbs_set])
                            s_sols.update([s for s in nbs_db[h_strip].get("Supportive",[]) if s in approved_supportive_set])
                primary_sol_col.append(", ".join(sorted(list(p_sols)))); supportive_sol_col.append(", ".join(sorted(list(s_sols))))
            summary_display_df["Primary Solutions"] = primary_sol_col; summary_display_df["Supportive Solutions"] = supportive_sol_col
            st.session_state.calculated_results["Primary Solutions"] = primary_sol_col; st.session_state.calculated_results["Supportive Solutions"] = supportive_sol_col
            _sei_dropdown_options = sorted(set(sol.strip() for cols in (primary_sol_col,supportive_sol_col) for row_val in cols for sol in str(row_val).split(",") if sol.strip()))
            cols_to_show = [c for c in ["Infrastructure","Asset","Impact model","Possible Hazards","Primary Solutions","Supportive Solutions"] if c in summary_display_df.columns]
        ...
        st.info("Filter and refine NbS selection by integrating SSF, SEI, and HIA.")
        _sei_dropdown_options_v2 = []
        if not st.session_state.calculated_results.empty:
            extracted_sols = set(st.session_state.get("approved_nbs_methods",[]))
            if show_supportive: extracted_sols.update(st.session_state.get("approved_supportive_methods",[]))
            _sei_dropdown_options_v2 = sorted(list(extracted_sols))

        sei_factors_list_v2 = ["Community Engagement","Cultural Preferences","Workforce Availability","Economic Viability","Long term O&M costs","Land Ownership","Regulatory Constraints"]
        ssf_labels_v2 = {"Slope instability":"Unstable/Steep Slopes","Limited vegetation and low quality of soil":"Poor Soil/Low Vegetation","Limited access for implementation":"Difficult Site Access","Cold temperatures":"Cold Temperatures / Frost Risk","Limited water availability":"Water Scarcity","Lack of connection to major services":"No Infrastructure/Services Access","Space Constraints":"Very Limited Space","Exposure to soil, water and/or air pollution":"Polluted Soil/Water/Air","Limitations due to high population density":"Urban/Densely Populated Area"}

        if _sei_dropdown_options_v2:
            selected_nbs_for_sei = st.selectbox("Select an NbS Solution to configure SEI factors:",_sei_dropdown_options_v2,key="sei_dropdown_v2")
            if selected_nbs_for_sei not in st.session_state.sei_lookup: st.session_state.sei_lookup[selected_nbs_for_sei] = {f:1 for f in sei_factors_list_v2}
        else:
            selected_nbs_for_sei = None

        edit_ssf_toggle = st.toggle("🛠️ Customize Site-Specific Feasibility (SSF) Rules",key="edit_ssf_toggle_v2")
        if edit_ssf_toggle:
            st.markdown("##### Edit SSF Rulebook")
            val_to_str = {100:"Highly Feasible",50:"Moderately Feasible",0:"Not Feasible"}; str_to_val = {v:k for k,v in val_to_str.items()}
            ssf_criteria_list = list(ssf_labels_v2.keys()); edit_data = []
            for method in sorted(list(set(_sei_dropdown_options_v2))):
                if method in st.session_state.ssf_lookup:
                    row_dict = {"NbS Method":method}
                    for crit in ssf_criteria_list:
                        crit_data = st.session_state.ssf_lookup[method].get(crit,{}); val = crit_data.get("Value",100) if isinstance(crit_data,dict) else 100
                        row_dict[crit] = val_to_str.get(val,"Highly Feasible")
                    edit_data.append(row_dict)
            if edit_data:
                col_config_ssf = {"NbS Method":st.column_config.TextColumn(disabled=True)}
                for crit in ssf_criteria_list: col_config_ssf[crit] = st.column_config.SelectboxColumn(options=["Highly Feasible","Moderately Feasible","Not Feasible"],required=True)
                edited_df_ssf = st.data_editor(pd.DataFrame(edit_data),column_config=col_config_ssf,hide_index=True,use_container_width=True,key="ssf_rule_editor_v2")
                for _, row in edited_df_ssf.iterrows():
            ...
            with col_input2:
                with st.container(border=True):
                    ...
                    if selected_nbs_for_sei:
                        ...
                        for f in sei_factors_list_v2:
                            current_val = st.session_state.sei_lookup[selected_nbs_for_sei].get(f,1)
                            val = st.select_slider(f,options=[1,2,3],value=current_val,key=f"sei_{selected_nbs_for_sei}_{f}",help="1: Favorable, 2: Neutral, 3: Unfavorable")
                            st.session_state.sei_lookup[selected_nbs_for_sei][f] = val
                    else:
                        st.info("Select an NbS solution from the dropdown above to configure SEI factors.")

        # Compute RPRI filtration
        st.session_state.filtered_nbs_pool = []; low_perf_pool = []
        if not st.session_state.calculated_results.empty and "Primary Solutions" in st.session_state.calculated_results.columns:
            unique_pairs = {}; method_to_hazards = {}
            for row_idx, row in st.session_state.calculated_results.iterrows():
                try: pri_score = float(row.get("PRI scores",0)) if row.get("PRI scores") is not None and str(row.get("PRI scores","")).strip() != "" else 0.0
                except: pri_score = 0.0
                asset_val = str(row.get("Asset","")).strip() or "—"; impact_val = str(row.get("Impact model","")).strip() or "—"
                infra_val = str(row.get("Infrastructure","")).strip() or ""; row_label = f"{asset_val} — {impact_val}"
                raw_hazards = row.get("Possible Hazards",[])
                if isinstance(raw_hazards,str): hazards = [h.strip() for h in raw_hazards.replace("[","").replace("]","").replace("'","").replace('"',"").split(",") if h.strip()]
                elif isinstance(raw_hazards,list): hazards = raw_hazards
                else: hazards = []
                hazards = [h for h in hazards if h in all_hazards_for_selector]
                p_raw = str(row.get("Primary Solutions","")); row_p_sols = [s.strip() for s in p_raw.split(",") if s.strip() and s.strip() in st.session_state.get("approved_nbs_methods",[])]
                s_raw = str(row.get("Supportive Solutions","")); row_s_sols = [s.strip() for s in s_raw.split(",") if s.strip() and show_supportive and s.strip() in st.session_state.get("approved_supportive_methods",[])]
                all_sols = list(dict.fromkeys(row_p_sols+row_s_sols))
                for h in hazards:
                    for sol in all_sols:
                        pair_name = f"{sol} method for {h}"; up_key = (row_idx,pair_name); is_prim = sol in row_p_sols
                        if up_key not in unique_pairs: unique_pairs[up_key] = {"original_pri":pri_score,"status":"scored","pair_name":pair_name,"method_only":sol,"row_label":row_label,"asset":asset_val,"impact_model":impact_val,"infrastructure":infra_val,"is_primary":is_prim}
                        elif unique_pairs[up_key]["original_pri"] < pri_score:
                            unique_pairs[up_key]["original_pri"] = pri_score
                            if is_prim: unique_pairs[up_key]["is_primary"] = True
                        if sol not in method_to_hazards: method_to_hazards[sol] = set()
            ...
            if not st.session_state.filtered_nbs_pool:
                if not unique_pairs: st.info("💡 **Strategy Setup:** Please click the **'Save'** buttons in the Step 7.1 tables above.")
                else:
                    st.warning("⚠️ No solutions strictly passed the 30% feasibility threshold. Showing all.")
                    st.session_state.filtered_nbs_pool = list(unique_pairs.values())
            else:
                st.success(f"Filtration complete: {len(st.session_state.filtered_nbs_pool)} solutions passed.")
        ...
        if st.session_state.filtered_nbs_pool:
            ...
            for _, r in df_final.head(5).iterrows():
                fig_spider.add_trace(go.Scatterpolar(r=[r["ssf"],r["sei"],r["hia"],r["ssf"]],theta=["Site Feasibility (SSF)","Socio-Economic (SEI)","Hazard Attenuation (HIA)","Site Feasibility (SSF)"],fill="toself",name=r["name"]))
            fig_spider.update_layout(polar=dict(radialaxis=dict(visible=True,range=[0,100])),showlegend=True)
            st.plotly_chart(fig_spider,use_container_width=True)

            with st.expander("📊 View Full Site-Specific Filtration Summary",expanded=False):
                filt_config = {"name":st.column_config.TextColumn("NbS Method"),"ssf":st.column_config.ProgressColumn("SSF (%)",format="%.0f%%",min_value=0,max_value=100),"sei":st.column_config.ProgressColumn("SEI (%)",format="%.0f%%",min_value=0,max_value=100),"hia":st.column_config.ProgressColumn("HIA (%)",format="%.0f%%",min_value=0,max_value=100),"total":st.column_config.ProgressColumn("Final Index (%)",format="%.0f%%",min_value=0,max_value=100)}
                st.dataframe(pd.DataFrame(st.session_state.filtered_nbs_pool)[["name","ssf","sei","hia","total"]].drop_duplicates(),column_config=filt_config,use_container_width=True,hide_index=True)

            if low_perf_pool:
                with st.expander(f"View {len(low_perf_pool)} methods scoring 30% or below",expanded=False):
                    for item in low_perf_pool: st.warning(f"**{item['name']}** is not recommended (Score: {item['total']:.1f}%).")

            st.markdown("---")
            st.subheader("Residual Risk — RPRI Ranking")

            def get_dynamic_color_rpri(rpri_val, eff_percent, min_val, max_val):
            ...
            def render_rpri_ranking(pool, show_row_badge=False):
                cons = {}
                for item in pool:
                    if not item.get("is_primary",False): continue
                    m = item["method_only"]
                    if m not in cons or item.get("rpri",0) > cons[m].get("rpri",0): cons[m] = item
                scored = sorted([i for i in cons.values() if i["status"]=="scored"], key=lambda x:(x["eff_percent"]==0, x["rpri"]))
                if not scored: st.info("No primary solutions found."); return
                valid_vals = [i["rpri"] for i in scored if i["eff_percent"]>0]
                mn,mx = (min(valid_vals),max(valid_vals)) if valid_vals else (0,0)
                for rank,item in enumerate(scored,1):
                    bg = get_dynamic_color_rpri(item["rpri"],item["eff_percent"],mn,mx); tc = "black" if bg in ["#fff176","#ffb74d","#aed581"] else "white"
                    pri_val = item.get("original_pri",0.0); rpri_val = item.get("rpri",0.0); delta_val = pri_val - rpri_val
                    row_badge_html = ""
                    if show_row_badge:
                        lbl = item.get("row_label","")
                        if lbl: row_badge_html = f'<span style="background:#e3f2fd;color:#1565c0;border-radius:4px;padding:1px 6px;font-size:0.78em;margin-left:6px;">{lbl}</span>'
            ...
            for item in st.session_state.filtered_nbs_pool:
                lbl = item.get("row_label","")
                if lbl and item.get("is_primary",False) and lbl not in seen_labels: seen_labels.append(lbl)
            ALL_LABEL = "🌐 All Rows Combined"; view_options = [ALL_LABEL] + seen_labels
            st.info("Use the selector below to view the RPRI ranking for a specific row, or select **All Rows Combined**.",icon="ℹ️")
            selected_view = st.selectbox("📋 View ranking for:",view_options,key="rpri_view_selector")
            if selected_view == ALL_LABEL:
                render_rpri_ranking(st.session_state.filtered_nbs_pool,show_row_badge=True)
            else:
                row_pool = [item for item in st.session_state.filtered_nbs_pool if item.get("row_label")==selected_view]
                render_rpri_ranking(row_pool,show_row_badge=False)

            st.divider()
            st.markdown("##### 🗺️ Cross-Row RPRI Heatmap")
            st.caption("Each cell shows the RPRI value for a given NbS method (row) across every Impact Model / Asset row (column). Green = low residual risk · Red = high residual risk.")
            hm_data = {}; hm_all_labels = []
            if not st.session_state.calculated_results.empty:
                for _, _cr_row in st.session_state.calculated_results.iterrows():
                    _a = str(_cr_row.get("Asset","")).strip() or "—"; _im = str(_cr_row.get("Impact model","")).strip() or "—"
                    _lbl = f"{_a} — {_im}"
                    if _lbl and _lbl not in hm_all_labels: hm_all_labels.append(_lbl)
            for item in st.session_state.filtered_nbs_pool:
                m = item.get("method_only",""); lbl = item.get("row_label",""); rv = item.get("rpri",None); st_val = item.get("status","")
                if not m or not lbl or st_val!="scored" or rv is None or not item.get("is_primary",False): continue
                if m not in hm_data: hm_data[m] = {}
                if lbl not in hm_data[m] or rv > hm_data[m][lbl]: hm_data[m][lbl] = rv
            hm_col_base = hm_all_labels if hm_all_labels else seen_labels

            if len(hm_data) >= 1 and len(hm_col_base) >= 1:
                sorted_methods = sorted(hm_data.keys(),key=lambda m:sum(hm_data[m].values())/len(hm_data[m]) if hm_data[m] else 0)
                ALL_COL_LABEL = "🌐 All Rows Combined"; col_labels = hm_col_base + [ALL_COL_LABEL]; row_labels = sorted_methods
                z_matrix = []; text_matrix = []
                for method in row_labels:
                    z_row = []; t_row = []; row_vals = []
                    for lbl in hm_col_base:
                        val = hm_data[method].get(lbl,None); z_row.append(val if val is not None else np.nan); t_row.append(f"{val:.2f}" if val is not None else "—")
                        if val is not None: row_vals.append(val)
                    if row_vals: agg = sum(row_vals)/len(row_vals); z_row.append(agg); t_row.append(f"{agg:.2f}")
                    else: z_row.append(np.nan); t_row.append("—")
                    z_matrix.append(z_row); text_matrix.append(t_row)
                def _rpri_cell_color(val):
                    if val is None or (isinstance(val,float) and np.isnan(val)): return "#f0f0f0"
                    stops = [(0.00,(27,94,32)),(0.14,(46,125,50)),(0.28,(174,213,129)),(0.50,(255,241,118)),(0.72,(255,183,77)),(0.86,(245,124,0)),(1.00,(211,47,47))]
                    t = max(0.0,min(1.0,val/5.0))
                    for i in range(len(stops)-1):
                        t0,c0 = stops[i]; t1,c1 = stops[i+1]
                        if t0 <= t <= t1:
                            r = (t-t0)/(t1-t0); rgb = tuple(int(c0[j]+r*(c1[j]-c0[j])) for j in range(3)); return f"rgb{rgb}"
                    return f"rgb{stops[-1][1]}"
                def _font_color(val):
                    if val is None or (isinstance(val,float) and np.isnan(val)): return "#888888"
                    return "white" if val>=3.5 or val<=0.7 else "black"
                def _short(label,max_len=28): return label if len(label)<=max_len else label[:max_len-1]+"…"
                short_col_labels = [_short(l) for l in col_labels]; fig_height = max(300,36+30*len(row_labels))
                tbl_cell_values = [row_labels]; tbl_fill_colors = [["#1565c0"]*len(row_labels)]; tbl_font_colors = [["white"]*len(row_labels)]
                for ci,col_lbl in enumerate(short_col_labels):
                    col_vals = [z_matrix[ri][ci] for ri in range(len(row_labels))]; col_texts = [text_matrix[ri][ci] for ri in range(len(row_labels))]
                    tbl_cell_values.append(col_texts); tbl_fill_colors.append([_rpri_cell_color(v) for v in col_vals]); tbl_font_colors.append([_font_color(v) for v in col_vals])
                hm_fig = go.Figure(go.Table(
                    columnwidth=[220]+[90]*len(short_col_labels),
                    header=dict(values=["<b>NbS Method</b>"]+[f"<b>{l}</b>" for l in short_col_labels],fill_color="#1565c0",font=dict(color="white",size=11),align=["left"]+["center"]*len(short_col_labels),height=36,line=dict(color="white",width=1)),
                    cells=dict(values=tbl_cell_values,fill_color=tbl_fill_colors,font=dict(color=tbl_font_colors,size=11),align=["left"]+["center"]*len(short_col_labels),height=28,line=dict(color="white",width=1))))
                hm_fig.update_layout(height=fig_height,margin=dict(l=0,r=0,t=10,b=10),paper_bgcolor="white")
                st.plotly_chart(hm_fig,use_container_width=True)
                st.markdown('<div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:4px;font-size:0.82em;align-items:center;"><span style="background:#1b5e20;color:white;padding:2px 8px;border-radius:4px;">Very low RPRI</span><span style="background:#aed581;color:black;padding:2px 8px;border-radius:4px;">Low–medium</span><span style="background:#fff176;color:black;padding:2px 8px;border-radius:4px;">Medium</span><span style="background:#ffb74d;color:black;padding:2px 8px;border-radius:4px;">Medium–high</span><span style="background:#d32f2f;color:white;padding:2px 8px;border-radius:4px;">High RPRI</span><span style="background:#f0f0f0;color:#888;padding:2px 8px;border-radius:4px;">— Not applicable</span></div>',unsafe_allow_html=True)
            else:
                st.warning("⚠️ Heatmap could not render. Ensure the RPRI ranking has been calculated and primary solutions are assigned.")
        else:
    ...
    # ══════════════════════════════════════════════════════════
    # SPECIFIC SITE ANALYSIS  (original specific-site app)
    # ══════════════════════════════════════════════════════════
    # Ensure current_view is correct when a site card is clicked
    st.session_state['current_view'] = 'specific_site'
