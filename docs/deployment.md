...
The production deployment is publicly accessible at **[nature-demo-dst.dic-cloudmate.eu](https://nature-demo-dst.dic-cloudmate.eu)**. No local installation is required for end users — open the URL in a browser. The application provides a public **Decision Support Tool** (no sign-up required) and a **High Resolution DST** upgrade (sign-up required — see [Authentication](user_guide/authentication.md) — which adds the pre-evaluated demonstrator-site configurations).
